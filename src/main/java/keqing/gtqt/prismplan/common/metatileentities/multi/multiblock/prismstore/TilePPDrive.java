package keqing.gtqt.prismplan.common.metatileentities.multi.multiblock.prismstore;
...
import keqing.gtqt.prismplan.network.Ae2uGuiHandler;
...
public class TilePPDrive extends AENetworkInvTile implements IChestOrDrive, IPriorityHost {
    ...
    private final PPDriveWatcher<IAEItemStack>[] invBySlot = new PPDriveWatcher[10];
    ...
    public int getCellStatus(final int slot) {
        ...
        final PPDriveWatcher handler = this.invBySlot[slot];
        if (handler == null) {
            return 0;
        }
                    ...
                    if (this.handlersBySlot[x] != null) {
                        for (IStorageChannel<? extends IAEStack<?>> channel : storageChannels) {
                            ...
                            if (cell != null) {
                                ...
                                final PPDriveWatcher<IAEItemStack> ih = new PPDriveWatcher(cell, is, this.handlersBySlot[x], this);
    ...
    public GuiBridge getGuiBridge() {
        return convertToGuiBridge(Ae2uGuiHandler.GUI_PP_DRIVE);
    }

    private static GuiBridge convertToGuiBridge(Ae2uGuiHandler handler) {
        try {
            return GuiBridge.valueOf(handler.name());
        } catch (IllegalArgumentException e) {
            return GuiBridge.GUI_DRIVE; // 安全回退值
        }
    }
